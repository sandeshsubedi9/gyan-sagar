...
const AdminLayout = async ({
  ...
  const isAdminAuthorized = cookieStore.get("admin_authorized")?.value === "true";
  ...
  const isUserDbAdmin = user?.role === "ADMIN";

  if (!isAdminAuthorized && !isUserDbAdmin) {
      ...
      <aside className="w-64 flex-col fixed inset-y-0 z-50 bg-slate-900 text-white hidden md:flex">
        <div className="px-6 py-5 border-b border-slate-700">
          ...
          {user?.name ? (
            ...
            <p className="text-xs text-slate-400 mt-2 truncate">Passkey Admin</p>
          )}
        </div>
