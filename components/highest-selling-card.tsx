...
import { BookOpen, GraduationCap, TrendingUp, User } from "lucide-react";
    ...
    return(
        <Link href={`/courses/${id}`}>
            <div className="group hover:shadow-md transition overflow-hidden border rounded-xl p-3 h-full bg-white">
                <div className="relative w-full aspect-video rounded-lg overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center">
                    {hasImage ? (
                        <Image 
                            src={imageUrl!}
                            alt={title}
